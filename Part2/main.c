...
void* BENSCHILLIBOWLCustomer(void* tid) {
  ...
  Order *order_ = malloc(sizeof(Order));
  order_->menu_item = PickRandomMenuItem();
  order_->customer_id = customer_id;
   AddOrder(bcb, order_);
 }
   return NULL;
}
...
void* BENSCHILLIBOWLCook(void* tid) {
  int cook_order_num = (int)(long) tid;
  ...
  Order *order;
   ...
   free(order);
	...
	printf("Cook #%d fulfilled %d orders\n", cook_order_num, orders_fulfilled);
	//return NULL;
}
